...
7. **Delete the task folder** — after harvesting is complete, remove the entire task directory (e.g., `.claude/SuperchargeAI/<agent_type>/<uuid>/`). The learnings now live in memory; the task folder is no longer needed
</workflow>
