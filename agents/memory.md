...
2. **Read existing memory** — read all files in `memory/project/`, `memory/methodology/behavior/`, and `memory/methodology/flows/` to understand current state
...
5. **Prune** — remove or consolidate memory that is outdated, superseded, or no longer relevant
6. **Delete the task folder** — after harvesting is complete, remove the entire task directory (e.g., `.claude/SuperchargeAI/<agent_type>/<uuid>/`). The learnings now live in memory; the task folder is no longer needed
</workflow>
